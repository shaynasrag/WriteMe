...
class TranscriptGUI():
    ...
    def show_submission(self):
        ...
        for entry in entries:
            Label(self.show_sub_frame, text = "Entry: " + str(count)).grid(row=row, sticky=W)          
            Label(self.show_sub_frame, text = "Person: " + entry._person).grid(row=row + 1, sticky=W)
            Label(self.show_sub_frame, text = "Anxiety Level: " + str(entry._anxiety) + "/3").grid(row=row + 2, sticky=W)
            Label(self.show_sub_frame, text = "Closeness: " + str(entry._communal_strength) + "/3").grid(row=row + 3, sticky=W)
            row += 3
            if entry._conflict is not None:
                Label(self.show_sub_frame, text = "Description of Conflict: " + entry._conflict).grid(row=row + 1, sticky=W)
                row += 1
            if entry._how_addressed is not None:
                Label(self.show_sub_frame, text = "How conflict was addressed: " + entry._how_addressed).grid(row=row + 1, sticky=W)
                Label(self.show_sub_frame, text = "Consent score: " + str(entry._consent) + "/1").grid(row=row + 2, sticky=W)
                Label(self.show_sub_frame, text = "Self soothe score: " + str(entry._self_soothe1) + "/1").grid(row=row + 3, sticky=W)
                Label(self.show_sub_frame, text = "Other soothe score: " + str(entry._other_soothe1) + "/1").grid(row=row + 4, sticky=W)
                Label(self.show_sub_frame, text = "Total communication score: " + str(entry._communication_score) + "/3").grid(row=row + 5, sticky=W)
                row += 5
            if entry._how_to_approach is not None:
                Label(self.show_sub_frame, text = "How to approach: " + entry._how_to_approach).grid(row=row + 1, sticky=W)
                Label(self.show_sub_frame, text = "Their side: " + entry._their_side).grid(row=row + 2, sticky=W)
                Label(self.show_sub_frame, text = "Another way to make the conversation feel safe: " + entry._how_to_approach).grid(row=row + 3, sticky=W)
                Label(self.show_sub_frame, text = "What to say: " + entry._how_to_approach).grid(row=row + 4, sticky=W)
                row += 4
                
            if entry._steps_to_secure is not None:
                Label(self.show_sub_frame, text = "Steps to security: " + entry._steps_to_secure).grid(row=row + 1, sticky=W)
                row += 1
            if entry._appreciate_other is not None:
                Label(self.show_sub_frame, text = "Appreciation of " + entry._person + ":" + entry._appreciate_other).grid(row=row + 1, sticky=W)
                row += 1
            if entry._appreciate_self is not None:
                Label(self.show_sub_frame, text = "Appreciation of self: " + entry._appreciate_self).grid(row=row + 1, sticky=W)
                row += 1
            if entry._gratitude is not None:
                Label(self.show_sub_frame, text = "Gratitude: " + entry._gratitude).grid(row=row + 1, sticky=W)
                row += 1
            if entry._support_from_others is not None:
                Label(self.show_sub_frame, text = "Support from others: " + entry._support_from_others).grid(row=row + 1, sticky=W)
                row += 1
            Label(self.show_sub_frame, text = "__________________________").grid(row=row + 1, sticky=W)
            row += 1
            count += 1
